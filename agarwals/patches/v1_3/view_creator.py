...
class ViewCreator:
    ...
    def create_current_sales_invoice_view(self):
        frappe.db.sql("""
    CREATE OR REPLACE VIEW `current_sales_invoice` AS
    SELECT
        `jv`.`name` AS `Bill No`,
            ...
            (
                (
                    SELECT
                        ...
                        `tsi`.`is_invoiced` AS `is_invoiced`,
                        `tsi`.`sales_team` AS `sales_team`
                    FROM
                ...
                LEFT JOIN
                    `tabJournal Entry Account` AS `tjea` ON `jv`.`name` = `tjea`.`reference_name`
                LEFT JOIN
                    `tabJournal Entry` AS `tje` ON `tjea`.`parent` = `tje`.`name`
                LEFT JOIN
                    (
                        SELECT
                            `tpe`.`reference_name` AS `pe_reference_name`,
                            SUM(`tpe`.`total_tds`) AS `pe_tds`,
                            SUM(`tpe`.`total_disallowance`) AS `pe_dis`,
                            SUM(`tpe`.`total_paid`) AS `pe_paid`
                        FROM
                        ...
                        GROUP BY
                            `tpe`.`reference_name`
                    ) AS `pen` ON `jv`.`name` = `pen`.`pe_reference_name`
            )
            LEFT JOIN
                `tabYearly Due` AS `tyd` ON `tyd`.`reference` = `jv`.`name`
            WHERE
    ...
    def create_sales_invoice_summary_view(self):
        frappe.db.sql("""
CREATE OR REPLACE VIEW `viewSales Invoice Summary` AS
    SELECT 
    ...
    def create_cancelled_bills_view(self):
        frappe.db.sql("""
CREATE OR REPLACE VIEW `viewcancelled_bills` AS
    select
        `tsi`.`name` AS `name`,
        `tsi`.`creation` AS `creation`,
        `tsi`.`modified` AS `modified`,
    ...
    def create_current_year_si_checklist_view(self):
        ...
        frappe.db.sql("""
                CREATE OR REPLACE VIEW `viewcurrent_year_si_checklist` AS
                select
    ...
    def create_bill_tracker_lastest_view(self):
    ...
    def process(self):
        ...
        self.create_bill_tracker_lastest_view()
