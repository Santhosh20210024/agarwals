...
def run_payment_entry():
    ...
    bank_transaction_records = frappe.db.sql(
        "SELECT name, bank_account, reference_number, date, unallocated_amount FROM `tabBank Transaction` WHERE status IN ('Pending','Unreconciled')  AND LENGTH(reference_number) > 4 AND deposit > 10 AND reference_number not like 'X0%' ORDER BY unallocated_amount DESC",
        as_dict=True
        )
