...
class PaymentEntryCreator:
    ...
    def update_payment_reference(self, sales_invoice, payment_entry, record):
        ...
        sales_doc.append('custom_reference', {'payment_entry': payment_entry.name,'paid_amount': payment_entry.paid_amount, 
                                                            ...
                                                            'allocated_amount': payment_entry.total_allocated_amount, 'utr_number': payment_entry.reference_no, 'utr_date': payment_entry.reference_date  })
    ...
    def process(self, bank_transaction_records, match_logic):
        ...
        for bank_transaction_record in bank_transaction_records:
            ...
            matcher_records = frappe.db.sql("""
                          ...
                          AND status is NULL order by payment_order ASC, tds_amount DESC , disallowance_amount DESC
...
def run_payment_entry():
    ...
    for i in range(0, len(bank_trans_records), n):
        batch_no = batch_no + 1
        frappe.enqueue(PaymentEntryCreator().process, queue='long', is_async=True, job_name="Batch" + str(batch_no), timeout=25000,
                       bank_transaction_records = bank_trans_records[i:i + n], match_logic = m_logic)


# ------------------ Changes Need to take place ----------------------

# Refactor
# Need to change as X00 series as per the Dr Agarwals need to change