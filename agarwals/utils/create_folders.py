...
PARENT_FOLDERS_LIST = ['Extract', 'Transform', 'Loding', 'Backup', 'Error', 'On-Error']
SUB_FOLDERS_LIST = ['Processed', 'Bank', 'Debtor_Payment', 'Claimbook', 'Bill']
...
def get_file_path(parent_folder_item = None,sub_folder_item = None,bank_item = None,tpa_item = None):
    ...
    if bank_item:
        path_components.append(bank_item)

    if tpa_item:
        path_components.append(tpa_item)
...
def folder_structure_creation():
    ...
    if ROOT_FOLDER not in get_file_list('all',{'is_folder':1}):
        create_new_folder(ROOT_FOLDER, "Home")
