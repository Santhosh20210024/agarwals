...
class PaymentEntryCreator:
    def __init__(self):
        self.bank_transaction_records = frappe.db.sql("SELECT name, bank_account, reference_number, date FROM `tabBank Transaction` WHERE status IN ('Pending','Unreconciled') AND deposit != 0  AND LENGTH(reference_number) > 4 AND deposit > 10 ORDER BY date",as_dict=True)
    ...
    def get_matched_claim_book_records(self, settlement_advice_claim_ids, match_with):
        ...
        for claim_record in self.claim_records:
            if not claim_record[match_with]:
                continue
    ...
    def get_matched_bill_and_logs(self, settlement_advice_record, bank_transaction_utr_number):
        ...
        if len(matched_bill_record) > 1:
            ...
            match_log.append({
                'log': f"bank_utr[{bank_transaction_utr_number}] > sa_utr[{settlement_advice_record.utr_number}] > sa_claim_id[{settlement_advice_record.claim_id}] > cb_al_number[{matched_claim_record[0]['al_number']}] > cb_final_bill_number[{matched_claim_record[0]['custom_raw_bill_number']}] > dbr_bill_number[{matched_bill_record[0]['name']}]",
        ...
        for matched_settlement_advice_record in matched_settlement_advice_records:
            ...
            if matched_sales_invoice_record.outstanding_amount < settled_amount:
                ...
                if matched_sales_invoice_record.outstanding_amount >= settled_amount + tds_amount:
                    payment_entry_created = self.create_payment_entry_record_and_update_bank_transaction(bank_transaction_record, matched_sales_invoice_record, bank_account, settled_amount, tds_amount)
                    settlement_advice_record.set('remark', 'Disallowance amount is greater than Outstanding Amount')
                else:
                    payment_entry_created = self.create_payment_entry_record_and_update_bank_transaction(bank_transaction_record,
                ...
                if payment_entry_created:
                    ...
                    settlement_advice_record.set('status', 'Warning')
                settlement_advice_record.save()
                frappe.db.commit()
            else:
                payment_entry_created = self.create_payment_entry_record_and_update_bank_transaction(bank_transaction_record,
    ...
    def process(self):
        for bank_transaction_record in self.bank_transaction_records:
            ...
            if not matched_settlement_advice_records:
                ...
                frappe.db.set_value('Bank Transaction', bank_transaction_record['name'], 'custom_advice_status', 'Not Found')
                frappe.db.commit()
                continue
