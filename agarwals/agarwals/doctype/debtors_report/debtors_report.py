...
class DebtorsReport(Document):
	def before_save(self):
		...
		sales_invoice_existing = True if len(frappe.get_list("Sales Invoice",filters={'name':self.bill_no})) == 1 else False
		if not sales_invoice_existing:
			frappe.msgprint("New Bill")
			try:
