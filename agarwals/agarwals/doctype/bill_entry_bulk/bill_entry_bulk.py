...
class BillEntry(Document):
	def update_bill_event(self, bill):
		bill_entry_log = frappe.get_doc(
			{'doctype': "Bill Entry Log", "bill": bill, "event": self.event,
			 "date": self.date, "mode_of_submission": self.mode_of_submission, "remark": self.remark})
	...
	def on_update(self):
		...
		for bill in bills:
			try:
				self.create_bill_entry_log(bill.bill)
