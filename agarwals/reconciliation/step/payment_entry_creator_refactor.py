...
class PaymentEntryCreator:
    ...
    def validate(self):
        ...
        if self.bt_doc.status not in ['Pending', 'Unreconciled']:
            error = 'Status Should be other then Pending, Unreconciled'
        if self.si_doc.status == 'Cancelled':
            error = 'Cancelled Bill'
        if self.si_doc.status== 'Paid':
            error = 'Already Paid Bill'
        if self.si_doc.total < (self.settled_amount + self.tds_amount + self.disallowance_amount):
