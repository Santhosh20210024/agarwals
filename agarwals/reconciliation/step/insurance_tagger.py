...
def tag_insurance(doctype: str) -> str:
    status = "Processed"
        ...
        if compressed_exclusion_patterns:
            frappe.db.sql("""
                        ...
                        """, values={'_tag': INSURANCE_TAG,
                                     'compressed_exclusion_patterns': compressed_exclusion_patterns})
            frappe.db.commit()
        else:
...
def process(args):
    ...
    ChunkOrchestrator().process(tag_insurance, step_id=args["step_id"], doctype=args["doctype"])
    return "Done"
