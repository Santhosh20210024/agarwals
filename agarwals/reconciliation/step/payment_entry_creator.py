...
class PaymentEntryCreator:
    ...
    def update_trans_reference(self, bt_doc, pe_doc):
        update_trans_timer = Timer().start("update_trans_reference")
    ...
    def process_rounding_off(self, pe_doc, si_doc):
        process_round_timer = Timer().start("process_rounding_off")
        deductions = []
    ...
    def get_company_account(self, bank_account_name):
        get_company_timer = Timer().start("get_company_account")
    ...
    def update_invoice_reference(self, si_doc, payment_entry, record):
        update_invoice_reference = Timer().start("update_invoice_reference")
    ...
    def process(self, bt_doc_records, match_logic,  chunk_doc):
        ...
        try:
            ...
            for transaction_record in bt_doc_records:
                t2 = Timer().start("Loop" + transaction_record['name'])
                ...
                for record in matcher_records:
                    t3 = Timer().start("Matcher Loop" + record.name)
                    try:
...
def process(args):
    try:
        ...
        with open("results.txt", "w") as file:
            file.write(f"{process_timer.print()}")
        process_timer.print()
    except Exception as e:
        process_timer = Timer().start("Payment Intial Process")
        chunk_doc = chunk.create_chunk(args["step_id"])
