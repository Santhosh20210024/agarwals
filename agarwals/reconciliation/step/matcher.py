...
class MatcherValidation:
    ...
    def is_valid(self):
        return (self._validate_advice() and
                self._validate_claim_amount() and
    ...
    def _validate_claim_amount(self):
        claim_amount = float(self.record.claim_amount)
        settled_amount = float(self.record.settled_amount)
        tds_amount = float(self.record.tds_amount)
        disallowed_amount = float(self.record.disallowed_amount)

        if self.record['advice'] and claim_amount and settled_amount:
            if claim_amount < settled_amount + tds_amount + disallowed_amount:
                Matcher.update_advice_status(self.record['advice'], 'Warning', 'Claim Amount is greater than the cumulative of settled, tds and disallowance')
                return False
        return True
...
class Matcher:
    ...
    def preprocess_entries(self):
        ...
        frappe.db.sql("""UPDATE `tabSettlement Advice` SET status = 'Open', remark = NULL, matcher_id = NULL WHERE status IN ('Not Processed', 'Open') OR remark = 'Not able to find Bill'""")
